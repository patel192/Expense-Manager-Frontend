            ...
            {navTabs.map((tab, i) => {
              ...
              return (
                <Link key={i} to={tab.path} className="relative group">
                  <div
                    className={`flex items-center gap-2.5 px-5 py-2.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 z-10 relative
                  ...
                  {isActive && (
                    <motion.div
                      layoutId="activeTab"
                      className="absolute inset-0 bg-gradient-to-br from-cyan-500 to-blue-600 rounded-xl shadow-lg shadow-cyan-500/20"
                      transition={{
                        type: "spring",
                        bounce: 0.2,
                        duration: 0.6,
        ...
        <AnimatePresence>
          {mobileMenuOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              className="lg:hidden absolute top-full left-0 w-full bg-[var(--surface-primary)] border-b border-[var(--border)] shadow-2xl overflow-hidden"
            >
                ...
                {navTabs.map((tab, i) => {
                  ...
                  return (
                    <Link
                      ...
                      className={`flex items-center gap-3 p-4 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all border shadow-sm
              ...
              <div className="px-6 pb-6 pt-4 border-t border-[var(--border)]/50 bg-[var(--surface-secondary)]/30">
                <button
                  ...
                  className="w-full flex items-center justify-center gap-3 p-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-rose-500 bg-rose-500/5 border border-rose-500/20 hover:bg-rose-500 hover:text-white transition-all shadow-sm"
                >
          ...
          {bottomTabs.map((tab, i) => {
            ...
            return (
              <button
                ...
                className={`flex flex-col items-center justify-center gap-1 transition-all duration-300 min-w-[50px]
                            ${isActive || isMoreActive ? "text-cyan-500 scale-110" : "text-[var(--text-muted)] hover:text-[var(--text-primary)]"}`}
              >
                {tab.isMore ? (
                  <>
                    <div
                      className={`p-2 rounded-xl ${isMoreActive ? "bg-cyan-500 text-white" : "bg-[var(--surface-secondary)]"}`}
                    >
                      {tab.icon}
                    </div>
                  ...
                  >
                    <div
                      className={`p-2 rounded-xl transition-all ${isActive ? "bg-cyan-500 text-white shadow-lg shadow-cyan-500/20" : "bg-transparent group-hover:bg-[var(--surface-secondary)]"}`}
                    >
                      {tab.icon}
                    </div>
                    <span className="text-[8px] font-black uppercase tracking-widest mt-1">
