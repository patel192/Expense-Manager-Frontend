...
import {
  ...
  Legend, LineChart, Line, ResponsiveContainer, ReferenceLine,
} from "recharts";
import { PlusIcon, XMarkIcon, TrashIcon } from "@heroicons/react/24/outline";
import { FaSyncAlt, FaPlusCircle, FaPauseCircle, FaTrashAlt } from "react-icons/fa";
import {
  FiTrendingUp, FiTrendingDown, FiDollarSign, FiPercent,
  FiCalendar, FiRepeat, FiBarChart2, FiList, FiAlignLeft,
  FiZap, FiRefreshCw, FiPlus,
} from "react-icons/fi";
...
/* ─── Source icon map ─── */
const sourceConfig = {
  Salary:       { color: "text-emerald-400", bg: "bg-emerald-500/15 border-emerald-500/25", label: "Salary" },
  Freelancing:  { color: "text-cyan-400",    bg: "bg-cyan-500/15 border-cyan-500/25",    label: "Freelancing" },
  Investments:  { color: "text-blue-400",    bg: "bg-blue-500/15 border-blue-500/25",    label: "Investments" },
  Other:        { color: "text-gray-400",    bg: "bg-gray-500/15 border-gray-500/25",    label: "Other" },
};
...
export const UserIncome = () => {
  ...
  const [stats, setStats] = useState({ totalIncome: 0, totalExpense: 0, netSavings: 0, savingsRate: 0 });
  ...
  const [recurringForm, setRecurringForm] = useState({ name: "", amount: "", frequency: "Monthly", startDate: "", category: "" });
  ...
  const computeAnalytics = (incomesData, expensesData) => {
    ...
    incomesData.forEach((inc) => { const m = new Date(inc.date).getMonth(); months[m].income += inc.amount; });
    expensesData.forEach((exp) => { const m = new Date(exp.date).getMonth(); months[m].expense += exp.amount; });
    const monthly = months.map((m, idx) => ({ month: new Date(0, idx).toLocaleString("default", { month: "short" }), income: m.income, expense: m.expense }));
    ...
    let suggestedTips = [];
    if (savingsRate < 20) suggestedTips = ["Track your expenses closely — small leaks sink big ships.", "Avoid impulse spending; wait a day before non-essential buys.", "Plan home-cooked meals to reduce daily spending."];
    else if (savingsRate < 40) suggestedTips = ["Good job! Start building a 6-month emergency fund.", "Automate savings or SIPs to stay consistent.", "Review recurring subscriptions and cancel unused ones."];
    else suggestedTips = ["Great savings! Consider diversifying investments.", "Focus on long-term growth via index funds or ETFs.", "Explore low-effort passive income opportunities."];
  ...
  const fetchData = async () => {
    ...
    } catch (error) { console.error("Error fetching incomes/expenses:", error); }
  ...
  const onSubmitIncome = async (data) => {
    const payload = { userID: data.userID, amount: Number(data.amount), source: data.source, date: data.date };
    try {
  ...
  const fetchRecurring = async () => {
    ...
    } catch (error) { console.error("Failed to load recurring payments", error); }
  };
  ...
  const handleRecurringSubmit = async (e) => {
    e.preventDefault();
    if (!recurringForm.name || !recurringForm.amount || !recurringForm.startDate) return alert("Please fill all required fields");
    try {
      setRecurringLoading(true);
      await axiosInstance.post("/recurring", { ...recurringForm, userId });
      setRecurringForm({ name: "", amount: "", frequency: "Monthly", startDate: "", category: "" });
      fetchRecurring();
    } catch (error) { console.error("Failed to add recurring payment", error); }
  ...
  const handleRecurringDelete = async (id) => {
    ...
    try {
      ...
      fetchRecurring();
    } catch (error) { console.error("Failed to delete recurring payment", error); }
  };
  ...
  /* ── Input class ── */
  const inputCls = "w-full px-3.5 py-2.5 rounded-xl bg-black/40 border border-white/10 text-gray-100 placeholder-gray-600 text-sm focus:outline-none focus:border-cyan-500/60 focus:ring-1 focus:ring-cyan-500/30 hover:border-white/20 transition-all duration-200";
  ...
  return (
    <div className="space-y-6 text-white">
      ...
      >
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold tracking-tight">Income Center</h1>
          <p className="text-gray-500 mt-1 text-sm">Track income, analyze trends, manage all records in one place.</p>
        </div>
        <button
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === "summary" && (
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { label: "Total Income",   value: `₹${stats.totalIncome.toLocaleString("en-IN")}`,  color: "text-emerald-400", bg: "bg-emerald-500/5",  border: "border-emerald-500/20", glow: "bg-emerald-400", icon: <FiTrendingUp size={17} /> },
                  { label: "Total Expenses", value: `₹${stats.totalExpense.toLocaleString("en-IN")}`, color: "text-rose-400",    bg: "bg-rose-500/5",     border: "border-rose-500/20",    glow: "bg-rose-400",    icon: <FiTrendingDown size={17} /> },
                  { label: "Net Savings",    value: `₹${stats.netSavings.toLocaleString("en-IN")}`,   color: "text-cyan-400",   bg: "bg-cyan-500/5",     border: "border-cyan-500/20",    glow: "bg-cyan-400",    icon: <FiDollarSign size={17} /> },
                  { label: "Savings Rate",   value: `${stats.savingsRate}%`,                           color: "text-amber-400",  bg: "bg-amber-500/5",    border: "border-amber-500/20",   glow: "bg-amber-400",   icon: <FiPercent size={17} /> },
                ].map((card, idx) => (
                  <motion.div
                    key={idx}
                  ...
                  >
                    ...
                    <div className="relative">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center mb-3 ${card.bg} border ${card.border}`}>
                        <span className={card.color}>{card.icon}</span>
                      </div>
                      <p className="text-[11px] font-medium text-gray-500 uppercase tracking-widest mb-1">{card.label}</p>
              ...
              >
                <div className="flex items-center gap-2 mb-4">
                  ...
                  <h3 className="text-sm font-semibold text-amber-300">Smart Finance Insights</h3>
                </div>
            ...
            >
              ...
              <div className="rounded-2xl bg-[#0d0f14]/80 border border-white/10 backdrop-blur-sm p-5">
                <div className="flex items-center gap-2 mb-5">
                  ...
                  <h3 className="text-sm font-semibold text-white">Income Trend & Average</h3>
                </div>
                <ResponsiveContainer width="100%" height={260}>
                  <LineChart data={monthlyIncomeTrend}>
                    ...
                    <ReferenceLine y={avgIncome} stroke="#3b82f6" strokeDasharray="4 4"
                      label={{ value: `Avg ₹${Math.round(avgIncome/1000)}k`, position: "right", fill: "#3b82f6", fontSize: 10 }} />
                    <Line type="monotone" dataKey="amount" stroke="#06b6d4" strokeWidth={2.5}
                      dot={{ r: 3, fill: "#06b6d4", strokeWidth: 0 }} activeDot={{ r: 6, fill: "#06b6d4" }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </motion.div>
          ...
          {activeTab === "records" && (
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="text-base font-semibold text-white">All Income Records</h3>
                  <p className="text-gray-500 text-xs mt-0.5">{incomes.length} records total</p>
                </div>
                <button
                  onClick={() => setIsModalOpen(true)}
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between mb-4">
                          <div>
                            <span className={`inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full border ${src.bg} ${src.color}`}>
                        ...
                        <div>
                          <p className={`text-2xl font-bold tracking-tight ${src.color}`}>
            ...
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">Manage your regular income & payment schedules</p>
                </div>
              </div>
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  <input type="text" name="name" placeholder="Payment name" value={recurringForm.name}
                    onChange={handleRecurringChange} className={inputCls} />
                  <input type="number" name="amount" placeholder="Amount (₹)" value={recurringForm.amount}
                    onChange={handleRecurringChange} className={inputCls} />
                  <select name="frequency" value={recurringForm.frequency}
                  ...
                  <input type="date" name="startDate" value={recurringForm.startDate}
                    onChange={handleRecurringChange} className={inputCls} />
                  <input type="text" name="category" placeholder="Category (optional)" value={recurringForm.category}
                    onChange={handleRecurringChange} className={inputCls} />
              ...
              <div className="rounded-2xl bg-[#0d0f14]/80 border border-white/10 backdrop-blur-sm overflow-hidden">
                <div className="px-5 py-4 border-b border-white/8 flex items-center justify-between">
                  <h3 className="text-sm font-semibold text-white">Your Recurring Payments</h3>
                ...
                {recurringPayments.length === 0 ? (
                  <div className="flex flex-col items-center gap-2 py-10 text-center">
                    ...
                    <p className="text-sm text-gray-600">No recurring payments added yet.</p>
                  </div>
                ) : (
                  <>
                    {/* Desktop table */}
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="border-b border-white/8">
                            {["Name", "Amount", "Frequency", "Start Date", "Category", "Actions"].map(h => (
                        ...
                        <tbody>
                          {recurringPayments.map((pay, i) => (
                            <motion.tr key={pay._id}
                              initial={{ opacity: 0, x: -12 }}
                              animate={{ opacity: 1, x: 0 }}
                              transition={{ delay: i * 0.04 }}
                              className="border-b border-white/5 hover:bg-white/3 transition-colors"
                            >
                              <td className="px-5 py-3.5 text-gray-200 font-medium">{pay.name}</td>
                              <td className="px-5 py-3.5 text-emerald-400 font-semibold">₹{pay.amount.toLocaleString("en-IN")}</td>
                              <td className="px-5 py-3.5">
                                <span className="text-xs px-2.5 py-1 rounded-full bg-cyan-500/10 border border-cyan-500/20 text-cyan-400">
                                  {pay.frequency}
                                </span>
                              </td>
                              <td className="px-5 py-3.5 text-gray-400 text-xs">{new Date(pay.startDate).toLocaleDateString("en-IN", { day: "2-digit", month: "short", year: "numeric" })}</td>
                              <td className="px-5 py-3.5 text-gray-500 text-xs">{pay.category || "—"}</td>
                              <td className="px-5 py-3.5">
                                <div className="flex items-center gap-2">
                                  <button onClick={() => alert("Pause feature coming soon")}
                                  ...
                                  <button onClick={() => handleRecurringDelete(pay._id)}
                    ...
                    <div className="sm:hidden divide-y divide-white/5">
                      {recurringPayments.map((pay) => (
                        <div key={pay._id} className="px-4 py-4 flex items-center justify-between gap-3">
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-200 truncate">{pay.name}</p>
                            <div className="flex items-center gap-2 mt-1">
                              <span className="text-emerald-400 text-xs font-semibold">₹{pay.amount.toLocaleString("en-IN")}</span>
                              <span className="text-[10px] px-2 py-0.5 rounded-full bg-cyan-500/10 text-cyan-400">{pay.frequency}</span>
                            </div>
                          </div>
      ...
      <Transition appear show={isModalOpen} as={Fragment}>
        <Dialog as="div" className="relative z-50" onClose={() => setIsModalOpen(false)}>
          ...
          <div className="fixed inset-0 flex items-center justify-center p-4">
            <Transition.Child as={Fragment}
              enter="ease-out duration-250" enterFrom="opacity-0 scale-95 y-4" enterTo="opacity-100 scale-100 y-0"
              ...
              <Dialog.Panel className="w-full max-w-md rounded-2xl bg-[#0f1115] border border-white/12 shadow-2xl overflow-hidden">
                ...
                <div className="flex items-center justify-between px-6 py-4 border-b border-white/8">
                  <div className="flex items-center gap-3">
                    ...
                    <Dialog.Title className="text-sm font-semibold text-white">Add New Income</Dialog.Title>
                  </div>
                ...
                <div className="px-6 py-5">
                  <form onSubmit={handleSubmit(onSubmitIncome)} className="space-y-4">
