...
const MetricCard = memo(({ title, value, icon, color, bg, border, glow }) => (
  <div
    className={`relative overflow-hidden rounded-2xl border p-5 bg-[var(--surface-primary)] ${border} shadow-sm backdrop-blur-md group hover:-translate-y-1 transition-all duration-300`}
  >
    <div
      className={`absolute -top-10 -right-10 w-24 h-24 rounded-full blur-3xl opacity-10 ${glow} transition-opacity group-hover:opacity-20`}
    />
    <div className="relative">
      <div
        className={`w-10 h-10 rounded-xl flex items-center justify-center mb-3 ${bg} border ${border} shadow-inner`}
      >
...
const AICard = memo(
  ({
    ...
    <div
      className={`rounded-[2rem] bg-[var(--surface-primary)] border ${borderColor} backdrop-blur-md overflow-hidden shadow-xl transition-all duration-300 hover:shadow-2xl`}
    >
      <div
        className={`flex items-center justify-between px-6 py-5 border-b ${borderColor} bg-[var(--surface-secondary)]/30`}
      >
        <div className="flex items-center gap-3">
          <div
            className={`w-10 h-10 rounded-xl flex items-center justify-center ${iconBg} shadow-inner`}
          >
        ...
        {onRefresh && (
          <button
            onClick={onRefresh}
            disabled={isLoading}
            className="w-8 h-8 rounded-lg flex items-center justify-center text-[var(--text-muted)] hover:text-cyan-500 hover:bg-cyan-500/10 transition-all active:scale-95"
          >
            <FiRefreshCw
              size={14}
...
const AIResult = ({ content }) => (
  ...
  >
    ...
    <div
      className="prose prose-invert prose-sm max-w-none text-[var(--text-secondary)] relative z-10
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
        ...
        <div
          className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-2xl border text-xs font-black uppercase tracking-widest shadow-lg transition-colors
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        />
        <div
          className={`relative overflow-hidden rounded-2xl border ${netSavings >= 0 ? "border-blue-500/20 bg-blue-500/5" : "border-rose-500/20 bg-rose-500/5"} backdrop-blur-md p-5 shadow-inner transition-all duration-300 hover:-translate-y-1`}
        >
          <div
            className={`absolute -top-10 -right-10 w-24 h-24 rounded-full blur-3xl opacity-10 ${netSavings >= 0 ? "bg-blue-500" : "bg-rose-500"}`}
          />
          <div className="relative">
            <div className="flex items-start justify-between mb-3">
              <div
                className={`w-10 h-10 rounded-xl flex items-center justify-center border shadow-inner ${netSavings >= 0 ? "bg-blue-500/10 border-blue-500/20" : "bg-rose-500/10 border-rose-500/20"}`}
              >
                <FiDollarSign
                  size={18}
                  className={
              ...
              <span
                className={`text-[9px] font-black uppercase tracking-widest px-2.5 py-1 rounded-full border ${netSavings >= 0 ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-500" : "bg-rose-500/10 border-rose-500/20 text-rose-500"}`}
              >
            ...
            <p
              className={`text-2xl font-black tracking-tight ${netSavings >= 0 ? "text-blue-500" : "text-rose-500"}`}
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        >
          {loadingRisk ? (
            ...
            <div
              className={`rounded-[1.5rem] p-5 border ${riskColors[riskData.riskLevel]?.border} ${riskColors[riskData.riskLevel]?.bg} shadow-inner`}
            >
              <div className="flex items-center justify-between mb-4">
                ...
                <span
                  className={`text-[10px] font-black px-3 py-1.5 rounded-full uppercase tracking-tighter ${riskColors[riskData.riskLevel]?.badge}`}
                >
              ...
              <div className="space-y-4">
                ...
                <div className="pt-2 border-t border-[var(--border)]">
                  <p
                    className={`text-xs font-bold ${riskColors[riskData.riskLevel]?.text}`}
                  >
                    ✨ {riskData.suggestion}
                  </p>
                </div>
            ...
            <div className="flex flex-col items-center justify-center py-4 text-center">
              <p className="text-xs font-medium text-[var(--text-muted)] mb-5 max-w-[80%]">
        ...
        >
          {loadingSavings ? (
            ...
            <div className="flex flex-col items-center justify-center py-4 text-center">
              <p className="text-xs font-medium text-[var(--text-muted)] mb-5 max-w-[80%]">
                Identify specific budget optimizations to maximize your monthly
                net surplus.
              </p>
              <AIButton
        ...
        >
          <div className="h-64 overflow-y-auto space-y-4 mb-5 pr-2 custom-scrollbar">
            ...
            {messages.map((msg, i) => (
              ...
              >
                <div
                  className={`max-w-[85%] px-4 py-3 rounded-2xl text-xs font-medium leading-relaxed shadow-lg
                  ${
                    msg.role === "user"
        ...
        <div className="rounded-[2.5rem] bg-[var(--surface-primary)] border border-[var(--border)] p-8 shadow-2xl relative overflow-hidden">
          ...
          <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3 relative z-10">
            {upcomingRecurring.map((item) => (
              <div
                key={item._id}
                className="flex items-center justify-between px-5 py-4 rounded-3xl bg-[var(--surface-secondary)]/30 border border-[var(--border)] hover:bg-[var(--surface-tertiary)]/20 transition-all border-dashed"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <p className="text-xs font-bold text-[var(--text-primary)] uppercase tracking-tight truncate max-w-[120px]">
                      {item.title}
                    </p>
        ...
        <div className="flex items-center justify-between mb-8 relative z-10">
          ...
          {allInsights.length === 0 && !loadingHistory && (
            <button
              ...
              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-cyan-500/10 border border-cyan-500/20 text-[10px] font-black text-cyan-500 uppercase tracking-widest hover:bg-cyan-500/20 transition-all active:scale-95"
            >
        ...
        {loadingHistory ? (
          ...
          <div className="flex flex-col items-center gap-4 py-20 text-center rounded-[2rem] bg-[var(--surface-secondary)]/10 border border-dashed border-[var(--border)]">
            ...
            <div className="space-y-1">
              <p className="text-xs font-bold text-[var(--text-primary)] uppercase tracking-widest leading-loose">
                No active telemetry available
              </p>
              <p className="text-[10px] font-medium text-[var(--text-muted)] max-w-[240px] mx-auto">
          ...
          <div className="grid gap-4 md:grid-cols-2 relative z-10">
            {allInsights.map((item) => (
              <div
                key={item._id}
                className="rounded-3xl bg-[var(--surface-secondary)]/30 border border-[var(--border)] p-6 hover:shadow-xl transition-all group border-dashed"
              >
