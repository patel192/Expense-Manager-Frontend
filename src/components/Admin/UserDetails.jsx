  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center py-24 gap-4">
        ...
        <p className="text-sm font-semibold text-cyan-400/60 uppercase tracking-widest animate-pulse">
  ...
  return (
    <div className="pb-10">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-sm font-bold text-[var(--muted)] uppercase tracking-[0.3em]">
            Subject Analysis
          </h1>
        ...
        >
            ...
            <div className="relative z-10 flex flex-col md:flex-row items-center gap-8 text-center md:text-left">
              <div className="relative">
                {user?.profilePic ? (
                  <img
                    src={user.profilePic}
                    alt={user.name}
                    className="w-32 h-32 rounded-3xl border-2 border-cyan-500/30 object-cover shadow-2xl p-1 bg-[var(--surface-tertiary)]/20"
                  />
                ) : (
              ...
              <div className="flex-1 space-y-3">
                ...
                <div className="flex flex-wrap items-center gap-4 justify-center md:justify-start pt-2">
                  <span
                    className={`px-4 py-1 rounded-xl text-[10px] font-black uppercase tracking-widest border ${
                      user.role === "Admin"
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              >
                ...
                <div
                  className={`p-4 rounded-2xl bg-[var(--surface-tertiary)]/10 ${card.accent} shadow-inner group-hover:scale-110 transition-transform`}
                >
                  {card.icon}
                </div>
              </motion.div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <motion.div className="p-8 rounded-[2.5rem] bg-[var(--surface-primary)] border border-[var(--border)] backdrop-blur-xl shadow-2xl">
              <h3 className="text-xs font-bold uppercase tracking-[0.3em] text-[var(--muted)] mb-8">
          ...
          <motion.div className="p-8 rounded-[2.5rem] bg-[var(--surface-primary)] border border-[var(--border)] backdrop-blur-2xl shadow-3xl">
            ...
            <div className="space-y-6 relative">
              ...
              {transactions.slice(0, 8).map((tx, idx) => (
                ...
                >
                  <span
                    className={`w-4 h-4 rounded-full absolute left-0 top-1/2 -translate-y-1/2 border-4 border-[var(--bg)] shadow-[0_0_10px_rgba(0,0,0,0.5)] z-20 transition-all duration-300 ${
                      tx.type === "income"
                        ? "bg-emerald-500 scale-110 group-hover:shadow-[0_0_15px_rgba(16,185,129,0.5)]"
                        : "bg-rose-500 scale-110 group-hover:shadow-[0_0_15px_rgba(244,63,94,0.5)]"
                    }`}
                  />
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-4 items-center p-5 rounded-2xl bg-[var(--surface-tertiary)]/10 border border-[var(--border)] hover:bg-[var(--surface-tertiary)]/20 transition-all">
                    ...
                    <div className="text-right flex items-center gap-3 md:justify-end">
                      <span
                        className={`px-2 py-0.5 rounded-lg text-[10px] font-bold border ${tx.type === "income" ? "border-emerald-500/20 text-emerald-400" : "border-rose-500/20 text-rose-400"}`}
                      >
                        {tx.type?.toUpperCase()}
                      </span>
                      <p
                        className={`text-xl font-black ${tx.type === "income" ? "text-emerald-400" : "text-rose-400"}`}
                      >
                        {tx.type === "income" ? "+" : "-"}₹
